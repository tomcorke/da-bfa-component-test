...
export const getSpec = (
  safeClassName?: WowClassSafeName,
  safeSpecName?: WowSpecSafeName
) => {
  const c = getClass(safeClassName);
  return (
    (c && c.specialisations.find(s => s.safeName === safeSpecName)) || undefined
  );
};

export const getSpecName = (safeSpecName?: WowSpecSafeName) => {
  const specs = classData.reduce(
    (allSpecs: WowSpecialisation[], c) => allSpecs.concat(c.specialisations),
    []
  );
  const spec = specs.find(s => s.safeName === safeSpecName);
  return (spec && spec.displayName) || undefined;
};
