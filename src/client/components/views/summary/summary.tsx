  ...
  return (
    <div className={STYLES.summaryColumn}>
      ...
      {selections.map(selection => (
        <SummaryItem
          key={`${selection.playerName}_${selection.choice}`}
          selection={selection}
        />
      ))}
    </div>
  ...
  return (
    <div className={STYLES.summaryDisplay}>
      ...
      <div className={STYLES.summaryColumns}>
        <SummaryColumn
          title="Tanks"
          selections={selections.filter(s => s.tags.includes("tank"))}
        />
        <SummaryColumn
          title="Healers"
          selections={selections.filter(s => s.tags.includes("healer"))}
        />
        <SummaryColumn
          title="Melee"
          selections={selections.filter(
            s => s.tags.includes("dps") && s.tags.includes("melee")
          )}
        />
        <SummaryColumn
          title="Ranged"
          selections={selections.filter(
            s => s.tags.includes("dps") && s.tags.includes("ranged")
          )}
        />
      </div>
    </div>
