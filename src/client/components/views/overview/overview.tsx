...
const OverviewView = ({
  ...
  const selectionsDisplay = battletags.map(battletag => {
    return <PlayerDisplay key={battletag} battletag={battletag} />;
  });
    ...
    <div className={STYLES.overview}>
      <div className={STYLES.summaryContainer}>
        ...
        {showLockedInSummary
          ? [
              <SummaryBox
                key="locked mains"
                title="Locked-in mains"
                selectionFilter={selection =>
                  selection.locked && selection.lockedChoice === "main"
                }
              />
            ]
          : [
              <SummaryBox
                key="selected-mains"
                title="Selected mains"
                selectionFilter={selection =>
                  selection.overviewSelection === "main"
                }
              />
            ]}
      </div>
      ...
      <div className={STYLES.options}>
        <label>
          <input
            type="checkbox"
            checked={showLockedInSummary}
            onChange={toggleShowLockedInSummary}
          />
      ...
      <div className={STYLES.selectionsContainer}>
        <div className={STYLES.selectionsHeader}>
          Player Selections ({battletags.length})
        </div>
        <div className={STYLES.selectionsBlurb}>
          Click on a player's chosen class to select it and update the summary
          above
        </div>
        {selectionsDisplay}
      </div>
