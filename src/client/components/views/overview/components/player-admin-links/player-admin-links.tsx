...
const PlayerAdminLinks = ({
  isAdmin,
  isSuperAdmin,
  onDeleteClick,
  playerCharacters,
  onChangeDisplayedCharacter
}: PlayerAdminLinksProps) => {
  const adminLinks: (() => JSX.Element)[] = [
    () => (
      <PlayerCharacterSelect
        key="player-character-select"
        characters={playerCharacters}
        onChange={onChangeDisplayedCharacter}
      />
    )
  ];
  const superAdminLinks: (() => JSX.Element)[] = [
    () => (
      <span
        ...
        className={STYLES.adminLinkDelete}
      >
        delete
      </span>
    )
