...
const PlayerSelection = ({
  ...
  const maxLevelCharacters = classCharacters.filter(
    char => char.level >= MAX_CHARACTER_LEVEL
  );
  const realmCharacters = maxLevelCharacters.filter(
    char => char.realm === config.realm
  );
  const guildCharacters = realmCharacters.filter(
    char => char.guild === config.guild
  );
  ...
  const profileTimestampString = `last updated ${
    profileTimestamp ? new Date(profileTimestamp).toLocaleString() : "(unknown)"
  }`;
...
${profileTimestampString}`;
    ...
    warningMessage = `Player has no max level characters of this class on ${
      config.realm
    }
...
${profileTimestampString}`;
  ...
  if (selection.locked) {
    ...
    selectionClasses.push(
      STYLES[`playerSelection__locked__${selection.lockedChoice || "none"}`]
    );
  ...
  if (overviewSelection) {
    ...
    selectionClasses.push(
      STYLES[`playerSelection__selected__${overviewSelection}`]
    );
  }
  ...
  return (
    <div
      ...
      onClick={onSelect}
    >
      <div className={STYLES.choiceNumber}>{choiceNumber}</div>
      ...
      <div className={STYLES.spec}>{specText}</div>
