...
const _getAuditLogSuccess = (data: APIAuditData) =>
  action(GET_AUDIT_LOG_SUCCESS, data);
...
export const getAuditLog = (opts: GetOverviewDataOptions = {}) => {
  return async (dispatch, getState: () => ApplicationState) => {
    if (!opts.noFeedback) {
      dispatch(feedbackActions.show("Getting audit data"));
    }
    dispatch(_getAuditLogStart());

    try {
      const response = await window.fetch(config.getAuditDataEndpoint, {
      ...
      if (!opts.noFeedback) {
        dispatch(feedbackActions.show("Error getting audit data.", "warning"));
      }
    }
  };
};
